...
for i = 1:INIT.num_groups
    DATA.X_gt(:, sum(nn(1:i))+1:sum(nn(1:i+1))) = DownsamplePiciture(DATA.YaleB_cell{1, INIT.start_person_id-1+i},...
                                                                    PIC.ori_height, ...
                                                                    PIC.ori_width, ...
                                                                    PIC.height, ...
                                                                    PIC.width);
    
    outliers_id = randperm(DATA.num_eachgroup(i), INIT.outliers_num_each);
    ...
    [X] = permute_face_randomly(DATA.X_gt(:, sum(nn(1:i))+1:sum(nn(1:i+1))),...
                                outliers_id,...
                                PIC.height, ...
                                PIC.width, ...
                                PIC.v_patches, ...
    ...
    DATA.X_tilde(:, sum(nn(1:i))+1:sum(nn(1:i+1))) = X;
    outliers_ID((index-1)*INIT.outliers_num_each+1:index*INIT.outliers_num_each) = sort(outliers_id)+sum(nn(1:i));
    index = index+1;
end
...
fprintf('\t-->Data Information:<--\n')
    fprintf(...
            ...
            '\n\toutlier_mod = '                num2str(INIT.perm_flag) '\n\n']                 ...
            )
