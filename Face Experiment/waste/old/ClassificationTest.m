...
for k = 1:num_groups
        [B_estimated, ~] = DPCP(X_sel{1,k}, c, delta, maxIter,epsilon_J,budget);
        Basis_estimated(:,:,k) = B_estimated;
end
...
MISSRATE = sum(IDX(:) ~= Label_gt(:))/(num_groups*n);
