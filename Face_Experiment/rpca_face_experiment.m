...
%% Robust PCA Recovery
lambda = 0.5/sqrt(max(size(X_tilde)));
mu = 10*lambda;
[X_RPCA, S] = RobustPCA(X_tilde, lambda, mu, 1e-6);
fprintf('RPCA recovered version: X_RPCA\n')
fprintf("\trank = %d\n", rank(X_RPCA))

%% Robust PCA + PMSDR 4-stage Pipeline
if use_PMSDR
    ...
    rrank = 8;
    U_esti = zeros(m*n, rrank, num_groups);
    for k = 1:num_groups
    ...
    for id = 1:num_groups
        ...
        lambda = 0.5/sqrt(max(size(XX)));
        mu = 10*lambda;
        [X_rpca, S] = RobustPCA(XX, lambda, mu, 1e-6);
        X_PMSDR_RPCA(:, allpoints_class_solved==id) = X_rpca;
    end
    fprintf('PMSDR_RPCA recovered version: X_PMSDR_RPCA\n')
    fprintf("\trank = %d\n", rank(X_PMSDR_RPCA))
end
%% Multiple Pairs Plots
run figure_plot
