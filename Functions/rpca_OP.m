function [L, C]= rpca_OP(M, rank_L, lambda)
% This is a function to solve mr_PCA with partial observation
% M is the input matrix with partial observation
% lambda is the input parameter
% L is the output low rank matrix
% C is the output column sparse matrix
% 'full_svd' if 1, use a full svd, otherwise a partial one

[m, n]=size(M);
Omega = ones(m,n); % Omega is the sampling set, i.e., Omega_(i,j)=1 if M_i,j is observed
