...
D = 50;
V = 120;
num_groups_set = [2,3,5,8,10]; % 不同的num_groups取值
outlier_ratio = 0.6;
...
for g_idx = 1: length(num_groups_set)
    ...
    for r_idx = 1:num_rank
        ...
        for sr_idx = 1:num_sr
            ...
            [X_solved_gtbases, outlier_class_solved_gtbases] = Solve_partial_LSR_(X_tilde, outliers_ID, U_gt);
            allpoints_class_solved_gtbases = zeros(V * num_groups, 1);
            for i = 1:V * num_groups
                if (ismember(i, outliers_ID) == 1)
                    allpoints_class_solved_gtbases(i) = outlier_class_solved_gtbases(outliers_ID == i);
                end
            end
            ...
            [recover_err_gtbases, recover_err_gtbases_detectedpart] = EvaluateRefined_v3(X_solved_gtbases, X_gt, outliers_ID, outliers_ID, num_groups, rrank);  % Noted that a same number of outlier in each group is needed
           ...
           %% Outlier Detection
            ...
            lambda = 0.95; 
            thres = 5e-4;
            [Inliers_id, Outliers_id] = Outlier_Detect_v4(X_tilde, length(outliers_ID), alpha, lambda);
           ...
           %% Reconstructed Recovery and Assessment
            ...
            [X_solved_esti, outlier_class_solved] = Solve_partial_LSR_(X_tilde, Outliers_id, U_esti);
            allpoints_class_solved = zeros(size(X_tilde, 2), 1); 
            for i = 1:size(X_tilde, 2)
                if (ismember(i, Inliers_id) == 1)
                    allpoints_class_solved(i, 1) = inlier_class_solved(Inliers_id == i);
                elseif (ismember(i, Outliers_id) == 1)
                    allpoints_class_solved(i, 1) = outlier_class_solved(Outliers_id == i);
                end
            end
            ...
            [recover_err, recover_err_detectedpart] = EvaluateRefined_v3(X_solved_esti, X_gt, outliers_ID, Outliers_id, num_groups, rrank); % Noted that a same number of outlier in each group is needed
