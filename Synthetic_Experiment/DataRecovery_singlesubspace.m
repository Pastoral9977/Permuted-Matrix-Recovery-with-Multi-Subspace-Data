...
if exist('DataRecovery_single_subspace.txt', 'file') == 2
    delete('DataRecovery_single_subspace.txt');
end
if use_diary
...
for g_idx = 1: length(num_groups_set)
    ...
    for r_idx = 1:num_rank
        ...
        for sr_idx = 1:num_sr
            ...
            [X_tilde, outliers_ID, inliers_ID] = Generate_observed_noisy_data(M_gt, outlier_ratio, shuffled_ratio, SNR);
            
            %% Recovery and Assessment
            fprintf('\tPart1: Recovery and Assessment\n');
            X_solved = Solve_single_subspace(X_tilde, outliers_ID, U_gt);
            [recover_err_gtbases_singlesubspace, ~] = EvaluateRefined(X_solved, X_gt, outliers_ID, outliers_ID, num_groups, rrank); 
